...
for(i in 1:nrow(sim.comb)){
    ...
    sim.res.e[,intervalo] <- tmp.hh$estocast[length(dt),,]
    sim.res.e.s[,intervalo] <- tmp.hh$sem.estocast[length(dt),,]
    sim.res.e.a[intervalo, ] <- tmp.hh$analitico
    }
...
for(i in 1:nrow(sim.comb)){
    intervalo <- (i*nrep-(nrep-1)):(i*nrep)
    tmp.hh.30 <- do.call("sim.compet2",
                   args=list(obj1=eval(parse(text=sim.comb[i,1])), obj2=eval(parse(text=sim.comb[i,2])),
                             obj3=eval(parse(text=sim.comb[i,3])),
                             data=experim1, nrep=nrep, dt=dt))
    sim.res[,intervalo] <- tmp.hh.30$estocast[length(dt),,]
    sim.res.s[,intervalo] <- tmp.hh.30$sem.estocast[length(dt),,]
    sim.res.a[intervalo, ] <- tmp.hh.30$analitico
    }
```
...
for(i in 1:length(sim.comb.ll)){
    ...
    tmp.ll <- do.call("sim.compet",
                   args=list(obj=eval(parse(text=sim.comb.ll[i])), 
                             data=experim1, nrep=nrep.ll))
    sim.res.e.ll[,intervalo] <- tmp.ll$estocast[ncol(experim1[[1]]),,]
    sim.res.e.s.ll[,intervalo] <- tmp.ll$sem.estocast[ncol(experim1[[1]]),,]
    sim.res.a.ll[intervalo, ] <- tmp.ll$analitico
    }
## Para 30 dias
...
for(i in 1:length(sim.comb.ll)){
    intervalo <- (i*nrep.ll-(nrep.ll-1)):(i*nrep.ll)
    tmp.ll.30 <- do.call("sim.compet",
                   args=list(obj=eval(parse(text=sim.comb.ll[i])), 
                             nrep=nrep.ll, dt = dt))
    sim.res.30.e.ll[,intervalo] <- tmp.ll.30$estocast[ncol(experim1[[1]]),,]
    sim.res.30.e.s.ll[,intervalo] <- tmp.ll.30$sem.estocast[ncol(experim1[[1]]),,]
    sim.res.30.a.ll[intervalo, ] <- tmp.ll.30$analitico
    }
```


```{r compet simul HL and LH}
##  HL: Simulacao com valores de r de Arcella altos
sim.comb.hl <- sapply(expand.grid(paste("fit",c(1,2,4,6,7), sep=""),
                        paste("fit.A",1:3, sep="")),
                     as.character)
dt <- diff(as.numeric(colnames(experim1[[1]])))
nrep.hl <- round(5000/nrow(sim.comb.hl))
sim.res.e.hl <- matrix(NA, 2, nrep.hl*nrow(sim.comb.hl)) ## para guardar resultados da simulacao estocastica
sim.res.e.s.hl <- matrix(NA, 2, nrep.hl*nrow(sim.comb.hl)) ## para guardar resultados da simulacao sem estocasticidade
sim.res.e.a.hl <- matrix(NA, nrep.hl*nrow(sim.comb.hl), 3) ## para guardar resultados dos valores no equilibrio analitico
for(i in 1:nrow(sim.comb.hl)){
    intervalo <- (i*nrep.hl-(nrep.hl-1)):(i*nrep.hl)
    tmp.lh <- do.call("sim.compet3",
                   args=list(obj1=eval(parse(text=sim.comb.hl[i,1])),
                             obj2=eval(parse(text=sim.comb.hl[i,2])),
                             dt=dt, nrep=nrep.hl))
    sim.res.e.hl[,intervalo] <- tmp.lh$estocast[length(dt),,]
    sim.res.e.s.hl[,intervalo] <- tmp.lh$sem.estocast[length(dt),,]
    sim.res.e.a.hl[intervalo, ] <- tmp.lh$analitico
    }

## LH: Simulacao com valores de r de Pyxidicula altos
sim.comb.lh <- sapply(expand.grid(paste("fit",c(1,2,4,6,7), sep=""),
                        paste("fit.P",1:3, sep="")),
                     as.character)
dt <- diff(as.numeric(colnames(experim1[[1]])))
nrep.lh <- round(5000/nrow(sim.comb.lh))
sim.res.e.lh <- matrix(NA, 2, nrep.lh*nrow(sim.comb.lh)) ## para guardar resultados da simulacao estocastica
sim.res.e.s.lh <- matrix(NA, 2, nrep.lh*nrow(sim.comb.lh)) ## para guardar resultados da simulacao sem estocasticidade
sim.res.e.a.lh <- matrix(NA, nrep.lh*nrow(sim.comb.lh), 3) ## para guardar resultados dos valores no equilibrio analitico
for(i in 1:nrow(sim.comb.lh)){
    intervalo <- (i*nrep.lh-(nrep.lh-1)):(i*nrep.lh)
    tmp.hl <- do.call("sim.compet4",
                   args=list(obj1=eval(parse(text=sim.comb.lh[i,1])),
                             obj2=eval(parse(text=sim.comb.lh[i,2])),
                             dt = dt, nrep=nrep.lh))
    sim.res.e.lh[,intervalo] <- tmp.hl$estocast[length(dt),,]
    sim.res.e.s.lh[,intervalo] <- tmp.hl$sem.estocast[length(dt),,]
    sim.res.e.a.lh[intervalo, ] <- tmp.hl$analitico
    }
```
...
Na subseções seguintes vão resultados de três previsões destas hipótese:

- A probabilidade de coexistência deve ser baixa com valores altos de $r$
- A probabilidade de coexistência deve ser mais alta com valores baixos de $r$
- Se a redução da velocidade de reprodução é uma estratégia com duas táticas ($r$ alto e baixo)
a escolha da tática de $r$ baixo pelas duas espécies deve ser um estado de equilíbrio comum
em um modelo de jogos.


## Probabilidades de coexistência com valores de r altos
...
Excluí dos experimentos as posteriores dos experimentos de competição 3 e 5, 
em que *Pyxidicula* e *Arcella* parecem ter valores de $r$ mais póximos de
culturas puras.
Com três experimentos de cultura pura por espécie e os cinco experimentos de competição
que mantive é possível fazer `r nrow(sim.comb)` combinações de distribuições *a posteriori*. 
De cada uma destas combinações de Tomei `r nrep` amostras.

Usei estes parâmetros sorteados para
calcular os tamanhos populacionais de três maneiras:

* Pelo modelo com estocasticidade, para a duração do experimento e após 30 dias
* Pelos tamanhos populacionais no equilíbrio do modelo determinístico
* Tamanhos populacionais pelo modelo determinístico, para a duração do experimento

A seções a seguir mostram os resultados para cada um destas simulações
...
```{r tabela extincoes}
t1 <- table(sim.res.e[1,]>0, sim.res.e[2,]>0)/(length(sim.res.e[1,]))
...
apenas de *Arcella* têm baixas probabilidades.

#### Prolongando a simulação para 30 dias

Não sei se este resultado ajuda em algo, mas quando prolongamos para 30 dias esta simulação os resultados
não mudam muito:

```{r tabela extincoes 30 dias}

t1m <- table(sim.res[1,]>0, sim.res[2,]>0)/length(sim.res[1,])
...
segundo o modelo Lotka-Volterra determinístico.

Também simulei as dinâmicas com este modelo determinístico,
no intervalo de duração do experimento


#### Solução no equilíbrio do modelo determinístico 


Os tamanho populacionais no equilíbrio são do modelo Lotka-Volterra são:
...
da mesma maneira (valores de $r$ e $K$ tomados dos experimentos monoespecíficos, demais parâmetros
dos experimentos de competição). A proporção destas combinações que resultam em tamanho populacionais
...
deve ocorrer a longo prazo, ou é pouquíssimo provável, 
caso as duas espécies mantivessem os valores de
$K$ e $r$ observados nas culturas mono-específicas. 


#### Simulação determinística até o fim do experimento

No entanto, se usamos o modelo determinístico para calcular os tamanhos populacionais 
até o fim do experimento, há uma probabilidade grande das duas espécies terem
populações maiores que zero:

```{r tabela extincoes sem estocast 11,3 dias}
t1m <- table(factor(sim.res.e.s[1,]>0, levels=c("FALSE","TRUE")),
             factor(sim.res.e.s[2,]>0 , levels=c("FALSE", "TRUE")))/length(sim.res.e.s[1,])
colnames(t1m) <- c("N(Pyx) = 0", "N(Pyx) > 0")
rownames(t1m) <- c("N(Arc) = 0", "N(Arc) > 0")
kable(t1m, digits=3,
...
Isto indica que pelo modelo determinístico o período de tempo do
experimento não é suficiente para se chegar ao equilíbrio determinístico.
Comparando com a seção anterior, vemos que as probabilidades de coexistência
pelo modelo determinístico são maiores do que com o modelo correspondente 
com estocasticidade.


## Probabilidades de coexistência com valores de r baixos

Fiz as mesmas simulações para o caso em os parâmetros $r$ e $K$
das duas espécies são os observados nos experimentos de competição em
que claramente houve redução do $r$ (experimentos 1, 2, 4, 6 e 7).


### Modelos com  estocasticidade

Com os valores de $r$ baixos a coexistência até o fim do experimento
torna-se muito mais provável, mesmo com estocasticidade ambiental.


```{r tabela extincoes LL}
t1 <- table(sim.res.e.ll[1,]>0, sim.res.e.ll[2,]>0)/length(sim.res.e.ll[1,])
colnames(t1) <- c("N(Pyx) = 0", "N(Pyx) > 0")
rownames(t1) <- c("N(Arc) = 0", "N(Arc) > 0")
kable(t1, digits=3,
...
#### Prolongando para 30 dias

```{r tabela extincoes LL 30 dias}
t1 <- table(sim.res.30.e.ll[1,]>0, sim.res.30.e.ll[2,]>0)/length(sim.res.30.e.ll[1,])
colnames(t1) <- c("N(Pyx) = 0", "N(Pyx) > 0")
rownames(t1) <- c("N(Arc) = 0", "N(Arc) > 0")
kable(t1, digits=3,
...
#### Probabilidade de persistência no equilíbrio determinístico

Mesmo usando o equilíbrio determinístico, a probabilidade
de coexistência é maior que zero, embora ainda
muito baixa.

```{r solucao analitica LL}
...
#### Simulação até o fim do experimento

Se usamos os valores baixos de $r$ sem estocasticidade 
há 100% de probabilidade de coexistência
no intervalo de tempo do experimento:


```{r tabela extincoes sem estocast no intervalo do experimento}
t1m <- table(factor(sim.res.e.s.ll[1,]>0, levels=c("FALSE","TRUE")),
             factor(sim.res.e.s.ll[2,]>0 , levels=c("FALSE", "TRUE")))/length(sim.res.e.s.ll[1,])
colnames(t1m) <- c("N(Pyx) = 0", "N(Pyx) > 0")
rownames(t1m) <- c("N(Arc) = 0", "N(Arc) > 0")
kable(t1m, digits=3,
...
## Probabilidades de persistência para LH e HL

Resultados para modelo com estocasticidade, 
pela duração do experimento

### Arcella com r alto e Pyxidiculla com r baixo

```{r tabela extincoes HL}
t1 <- table(factor(sim.res.e.hl[1,]>0, levels=c("FALSE","TRUE")),
             factor(sim.res.e.hl[2,]>0 , levels=c("FALSE", "TRUE")))/length(sim.res.e.hl[1,])
colnames(t1) <- c("N(Pyx) = 0", "N(Pyx) > 0")
rownames(t1) <- c("N(Arc) = 0", "N(Arc) > 0")
kable(t1, digits=3,
      caption="Proporção de simulações do modelo de competição estocástico
 com valores de r altos para *Arcella* e baixos para *Pixydiculla*
em que a população de cada espécie chegou ao fim de 11.3 dias com tamanhos maiores que zero.")
```

### Pyxidiculla com r alto e Arcella  com r baixo

```{r tabela extincoes LH}
t1 <- table(factor(sim.res.e.lh[1,]>0, levels=c("FALSE","TRUE")),
             factor(sim.res.e.lh[2,]>0 , levels=c("FALSE", "TRUE")))/length(sim.res.e.lh[1,])
colnames(t1) <- c("N(Pyx) = 0", "N(Pyx) > 0")
rownames(t1) <- c("N(Arc) = 0", "N(Arc) > 0")
kable(t1, digits=3,
      caption="Proporção de simulações do modelo de competição estocástico
 com valores de r altos para *Pixydiculla* e baixos para *Arcella*
em que a população de cada espécie chegou ao fim de 11.3 dias com tamanhos maiores que zero.")
```


## Equilíbrios em um jogo com duas estratégias


Vamos supor que cada espécie tem a tática de manter $r$ alto ou baixo,
...
Simulei 100 combinações de parâmetros para cada uma das `r nrow(sim.comb)`
combinações possíveis das 5 posteriores dos experimentos de competição com
as 3 posteriores de cada experimento de cultura pura. 

As simulações correram até o tempo que duraram os experimentos de competição 
( `r max(as.numeric(colnames(experim1[[1]])))` dias), usando os intervalos 
reais de medição, que variaram entre `r min(diff(as.numeric(colnames(experim1[[1]]))))` e
...
Abaixo tabela com estas probabilidades para *payoffs* baseados na probabilidade de persistência
...
```{r prob strategies pop persist 30 days}
f1 <- function(x, i, j, k){
    apply(x[[i]][,j:k], 2, mean)
}
pers.m2 <- matrix(
...
Uma possibilidade: calcular os componentes da variação dos atamnhos populacionais
* Na duração do experimento (11,3 dias) a probabilidade de coexistência
é  próxima de um se usamos os valores de $r$ e $K$ dos experimentos de competição,
com ou sem estocasticidade. Este resultado se mantém na simulação estocástica se prolongamos as simulações para 30 dias.
No  equilíbrio do modelo determinístico a coexistência não é possível, o que sugere que a coexistência é um transiente muito duradouro.
* A probabilidade de coexistência cai nas simulações estocásticas, comparadas com
as determinísticas. Isso indica que a estocasticidade está aumentando o risco
de extinção das populações.
...
têm maiores probabilidades. Também há variações importantes nestes resultados em função
do critério de estabilidade usado e de quanto tempo a simulação corre para
calcular os *payoffs*.
* As probabilidades de persistência de *Pyxidiculla* são altas em todas as combinações de estratégias.
Já para *Arcella* são bem menores para a combinação HH, e em geral um pouco menor para as outras.
Importante notar que as probabilidades de persistência em 11.3 dias são próximas de 1 para
as duas espécies (exceto para *Arcella* em HH).
